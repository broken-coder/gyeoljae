...
> gyeoljae is early-stage. `v0.2.0-rc.2` ships the core library, `poll`/`listen`/`watch` CLIs, notifier, Slack Socket Mode listener, and GitHub Issues adapter. The watch CLI uses a local chat outbox but performs live GitHub label/comment/close transitions when it sees workflow markers.
